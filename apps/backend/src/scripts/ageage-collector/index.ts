...
async function checkDatabaseConnection() {
  try {
    ...
    console.error('データベースサーバーが db:5432 で実行されていることを確認してください。');
 ...
 * 市町村名と郵便番号を分離する
 */
function separateZipCodeAndMunicipality(input: string): { municipalityName: string, zipCode: string | null } {
 ...
 */
async function getOrCreateMunicipality(name: string, zipCode?: string): Promise<number> {
  // 市町村名と郵便番号を分離
  const { municipalityName, zipCode: extractedZipCode } = separateZipCodeAndMunicipality(name);
  ...
  if (existingMunicipality) {
    ...
    if (finalZipCode && !existingMunicipality.zipcode) {
      ...
      console.log(`市町村の郵便番号を更新しました: ${municipalityName}, ${finalZipCode}`);
    }
  ...
  console.log(`新しい市町村を作成しました: ${municipalityName}, ${finalZipCode || 'なし'}`);
 ...
 * 取得したMeshiデータを保存する
 */
async function saveMeshiData(meshiData: Awaited<ReturnType<typeof scrapeAgeagePage>>) {
  ...
  for (const meshi of meshiData) {
    ...
    if (meshi.municipality) {
      municipalityId = await getOrCreateMunicipality(meshi.municipality, meshi.zipCode);
    }
    ...
    if (!pageArg) {
      ...
      while (hasMoreData) {
        ...
        if (pageResults.length === 0) {
          ...
          console.log(`ページ ${currentPage}: ${pageResults.length}件のデータを取得・保存しました`);
          currentPage++;
        }
      }
    } else {
